...
//get all gala names
$children = find_all_children($session_user['Id']);

// $sample_child = find_user_by_id(mysqli_fetch_assoc($children)['child_id']);
// echo 'sample child name' . $sample_child['full_name'];
        ?>
        <!DOCTYPE html>
        <html>
        <head>
    ...
    <header class="w3-container" style="padding-top:22px">
        <h5><b><i class="fa fa-users"></i> Children</b></h5>
    </header>


<div class = "w3-container">
    <td><a href="<?php echo url_for("parent/add_child.php");?>" class="w3-button w3-teal">New Child <i class="fa fa-light fa-plus"></i></a></td>
</div>
<div class="w3-container">
<div class="w3-panel">
    ...
    while($child = mysqli_fetch_assoc($children)){
        ...
        $user = find_user_by_id($child_id);
        ?>
     <tr>
    <td><?php echo h($user['full_name']); ?></td>
    <td><?php echo h($user['email']); ?></td>
    <td><a href="<?php echo url_for("parent/view_child.php?id=".$user['Id']);?>" class="w3-button w3-teal">View</a></td>
    <td><a href="<?php echo url_for("parent/edit_child.php?id=".$user['Id']);?>" class="w3-button w3-teal">Edit</a></td>
    <td><a href="<?php echo url_for("parent/delete_child.php?id=".$user['Id']);?>" class="w3-button w3-red">Delete</a></td>
    </tr>
    <?php } ?>
    </table>
    </div>
